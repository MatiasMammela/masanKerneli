...
void heap_init()
{
    ...
    heap = (heap_t *)PHYS_TO_VIRT(pmm_alloc(sizeof(heap_t)));
    if (heap == NULL)
    {
...
void *malloc(size_t size)
{
    ...
    heap_block *new_block = (heap_block *)PHYS_TO_VIRT(pmm_alloc(size + sizeof(heap_block)));
    if (!new_block)
    {
        return NULL;
    }
